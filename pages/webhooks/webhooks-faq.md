...
If a webhook delivery fails due to a timeout, no response, or a server error (5xx), TutorCruncher will automatically retry up to 4 times using exponential backoff (after 1s, 10s, 100s, and 1000s). Redirects and client errors (4xx, except 429) are not retried. If an endpoint's failure rate exceeds 20% over a sustained period, it will be automatically disabled.
